  ...
  useEffect(()=>{
    ...
    sty.textContent=`
      *{box-sizing:border-box;margin:0;padding:0}
      ...
      .stock-card:hover{transform:translateY(-3px);border-color:rgba(0,212,255,.45)!important;box-shadow:0 8px 28px rgba(0,212,255,.08)}
  ...
  const load = useCallback(async()=>{
    ...
    try {
      ...
      for(const r of flat){
        ...
        const d={price:r.regularMarketPrice,change:r.regularMarketChange,changePct:r.regularMarketChangePercent,volume:r.regularMarketVolume,prevClose:r.regularMarketPreviousClose,marketCap:r.marketCap};
        ...
        <main style={{padding:'20px 24px',maxWidth:1700,margin:'0 auto'}}>
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16,marginBottom:20}}>
            {[
              ...
              return (
                <div key={sym} className="card" style={{background:C.card,borderRadius:14,padding:'20px 24px',border:`1px solid ${d?up?C.gain+'33':C.loss+'33':C.border}`,position:'relative',overflow:'hidden'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,marginBottom:20}}>
            {[
              ...
              <div key={sub} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:13,overflow:'hidden'}}>
                ...
                {items.map((s,i)=>(
                  <div key={s.s} onClick={()=>navigate(`/stock/${s.s}`)} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 20px',borderBottom:i<4?`1px solid ${C.border}`:'none',cursor:'pointer',position:'relative',overflow:'hidden'}}>
          ...
          <div style={{marginBottom:12,color:C.dim,fontSize:11,display:'flex',gap:12}}>
            <span>מציג <strong style={{color:C.text}}>{enriched.length}</strong> מניות · לחץ על מניה לניתוח מפורט</span>
            {withData.length>0&&<><span>·</span><span style={{color:C.gain}}>▲ {withData.filter(s=>s.changePct>0).length}</span><span>·</span><span style={{color:C.loss}}>▼ {withData.filter(s=>s.changePct<0).length}</span></>}
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(215px,1fr))',gap:11}}>
            {enriched.map((s,idx)=>{
              ...
              return (
                <div key={s.s} className="stock-card" onClick={()=>navigate(`/stock/${s.s}`)} style={{background:C.card,borderRadius:10,padding:'14px 15px',border:`1px solid ${s.has?(up?C.gain+'22':C.loss+'22'):C.border}`,position:'relative',overflow:'hidden',transition:'transform .18s,border-color .18s,box-shadow .18s',animationDelay:`${Math.min(idx*.01,.25)}s`}}>
          ...
          <footer style={{marginTop:36,paddingTop:18,borderTop:`1px solid ${C.border}`,textAlign:'center',color:C.dim,fontSize:11,lineHeight:2}}>
            <div>נתונים מ-Yahoo Finance · הנתונים לצורך מידע בלבד · אין זה ייעוץ השקעות</div>
          </footer>
        </main>
      )}
    </div>
